...
export default function From({ onClick, fromAddress }) {
    ...
    const handleClick = (input) => {
        onClick(input)
        document.getElementById("search-from").value = `${input.subdistrict_name} - ${input.city_name}`
        setShowSearchResult(false)
    }
        ...
        <>
            <div className="m-3 flex-initial w-6/12 xl:w-96">
                ...
                <input
                    ...
                    onClick={(e) => { 
                        if (e.target.value.length < 3 && fromAddress == '') {
